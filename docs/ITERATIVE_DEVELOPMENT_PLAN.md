...
| H6 — oracle validation | In progress — V0.2 profile contract complete | Normalized trace format, runners for Ramulator 2.1 and DRAMSys where supported, metric comparer, stored reference configurations | Ubuntu CI builds and runs the pinned references. `hbm2_2000` makes the request, organization, timing and mapping contract auditable; V0.3 removes the recorded data-bus and DRAMSys stock-spec differences, then adds strict arrival/burst/completion alignment and numerical error targets. |
